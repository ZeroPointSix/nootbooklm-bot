...
- 内置 NotebookLM tool provider：`list_tools()` 暴露 32 个业务工具，`call_tool()` 在本仓库 handler 内执行
...
登录完成后调用 provider health 证明成功。`/notebook status` 也走同一套健全逻辑，返回后端、摘要和 profile/storage/google/notebooklm origin 等分层检查结果。
...
测试覆盖 32 个工具发现、通用调用、未知工具、畸形参数、工具循环上限、上游失败、敏感字段脱敏、token 重放、过期、非法状态转换、跨进程会话、Profile 权限、认证失败回滚和内置 provider health。
